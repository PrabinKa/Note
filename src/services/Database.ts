...
export async function fetchArchiveNotesFromDatabase(
  callback: (temp: Note[]) => void,
) {
  (await db).transaction(tx => {
...
export async function deleteArchiveNotes(id: string): Promise<boolean> {
  return new Promise(async resolve => {
    (await db).transaction(tx => {
      tx.executeSql(
        'DELETE FROM archive_note WHERE id = ?',
        [id],
        (tx, results) => {
          const isDeleted = results.rowsAffected > 0;
          resolve(isDeleted);
...
export async function fetchTrashNotesFromDatabase(
  callback: (temp: Note[]) => void,
) {
  (await db).transaction(tx => {
    tx.executeSql('SELECT * FROM trash_note', [], (tx, results) => {
      let temp = [];
      for (let i = 0; i < results.rows.length; ++i) {
        temp.push(results.rows.item(i));
      }
...
export async function deleteTrashNotes(id: string): Promise<boolean> {
  return new Promise(async resolve => {
    (await db).transaction(tx => {
      tx.executeSql(
        'DELETE FROM trash_note WHERE id = ?',
        [id],
        (tx, results) => {
          const isDeleted = results.rowsAffected > 0;
          resolve(isDeleted);
